...
import cn.yinxm.app.applist.ui.MainActivity;
...
public class HomeKeyDownReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        Log.i("yinxm", "[HomeKeyDownReceiver.onReceive] action="+action);
        if (action.equals(Intent.ACTION_CLOSE_SYSTEM_DIALOGS)) {
            String reason = intent.getStringExtra(SYSTEM_REASON);
            Log.i("yinxm", "reason="+reason);
            if (reason != null) {
                if (reason.equals(SYSTEM_HOME_KEY)) {
                    // home key处理点
                    long currentTime = System.currentTimeMillis();
                    Log.d("yinxm", "firstTime="+firstTime+", currentTime="+currentTime);
                    if (currentTime - firstTime > 2000) {//连续按键时间小于2s才有效
//                        Toast.makeText(context,"再按一次启动work_ec", Toast.LENGTH_SHORT).show();
                        ToastUtil.showTextToast(context, "再按一次启动已安装应用列表");
                        firstTime = currentTime;
                    } else {
                        Log.d("yinxm", "启动work_ec");
//                        Toast.makeText(context,"work_ec启动中...", Toast.LENGTH_SHORT).show();
                        ToastUtil.showTextToast(context, "应用列表启动中...");
